...
const ReportesMarcasView = {
  ...
  formatQty(value) {
    const n = Number(value);
    if (Number.isNaN(n)) return '—';
    return n.toLocaleString('es-SV', { maximumFractionDigits: 4 });
  },

  formatPct(value) {
    const n = Number(value);
    if (Number.isNaN(n)) return '—';
    return `${n.toLocaleString('es-SV', { maximumFractionDigits: 1 })}%`;
  },
      ...
      options: {
          ...
          tooltip: {
            callbacks: {
              label(ctx) {
                ...
                return `${ctx.dataset.label}: ${v.toLocaleString('es-SV', { maximumFractionDigits: 4 })}`;
              },
            },
          },
        },
        scales: {
          ...
          yUnits: {
            ...
            ticks: { callback: (v) => Number(v).toLocaleString('es-SV', { maximumFractionDigits: 0 }) },
          },
        },
      },
    });
