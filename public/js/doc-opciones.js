 ...
 */
const DocOpciones = {
  /** Prefijo telefónico WhatsApp (SV FS-SV). */
  WHATSAPP_DIAL_CODE: '503',
  ...
  async solicitarTelefonoWhatsapp(opts = {}) {
    const dial = String(opts.dialCode || this.WHATSAPP_DIAL_CODE || '503').replace(/\D/g, '') || '503';
  ...
  async fetchPrintableHtml(coddoc, correlativo, opts = {}) {
    ...
    const footerNote =
      tipodoc === 'COT'
        ...
        : opts.footerNote || 'Documento generado por FS ERP';
  ...
  abrirWhatsapp(telefono8, text) {
    const dial = this.WHATSAPP_DIAL_CODE || '503';
  ...
  async imprimir(coddoc, correlativo, row) {
    ...
    const titulo = String(row?.DESDOC || h.DESDOC || tipodoc || 'Documento').trim();
    const footerNote =
      tipodoc === 'COT' ? 'Cotización — documento sin validez fiscal' : 'Documento generado por FS ERP';
