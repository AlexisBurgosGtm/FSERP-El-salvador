...
/** Convierte monto a letras (dólares) para tickets. */
function amountInWordsGt(value) {
  ...
  const moneda = entero === 1 ? 'DOLAR' : 'DOLARES';
  if (centavos > 0) {
...
function buildPrintContext({ empresa, header, lines, title, footerNote, felUrlBase } = {}) {
  ...
  return {
    ...
    FOOTER: footerNote || 'Documento generado por FS ERP',
    EMPRESA: {
...
const FEL_TICKET_HTML = `<div class="fel-ticket">
  ...
  <div class="fel-obs fel-c">
    ...
    <div>POR CADA CHEQUE RECHAZADO SE COBRARÁ $ 125.00 POR GASTOS ADMINISTRATIVOS</div>
    {{/DOC.OBS}}
  </div>
...
function samplePrintContext() {
  return buildPrintContext({
    ...
    footerNote: 'Vista previa — FS ERP',
  });
}
