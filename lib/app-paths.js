...
/** Contador de compilación (writable; junto al .exe en empaquetado). */
function buildCounterPath() {
  return path.join(getDataRoot(), 'build-counter.json');
}

/** Meta de build para API y login (writable). */
function buildMetaWritablePath() {
  return path.join(getDataRoot(), 'build-meta.json');
}
